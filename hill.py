...
# --------------------------------------------------------------------------------
# Data loading
train_url = "datasets/feature_number/hill/Hill_Valley_without_noise_Training.data"
test_url = "datasets/feature_number/hill/Hill_Valley_without_noise_Testing.data"
...
# Drop rows with NaN values which may occur due to coercion
X_train = X_train.dropna()
y_train = y_train[X_train.index]  # Align y with X after dropping NaNs
X_test = X_test.dropna()
y_test = y_test[X_test.index]      # Align y with X after dropping NaNs

# Ensure labels are encoded as numeric
...
y_train = y_train.astype(np.int64)  # Ensure type is integer
X_test = X_test.values
y_test = y_test.astype(np.int64)    # Ensure type is integer
...
# Adjust data input size
X_train_gsknn = X_train_tensor.T  # Shape from [d, n] to [n, d]
X_test_gsknn = X_test_tensor.T    # Shape from [d, m] to [m, d]
...
output_dir = "outputs/feature_number/hill"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "hill.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "hill.png")
