...
#                                 Data Loading
# --------------------------------------------------------------------------------
# Read the Madelon dataset
url_train_data = "datasets/feature_number/madelon/MADELON/madelon_train.data"
url_train_labels = "datasets/feature_number/madelon/MADELON/madelon_train.labels"
url_valid_data = "datasets/feature_number/madelon/MADELON/madelon_valid.data"
url_valid_labels = "datasets/feature_number/madelon/madelon_valid.labels"

# Read the data
train_data = pd.read_csv(url_train_data, sep='\s+', header=None)
train_labels = pd.read_csv(url_train_labels, sep='\s+', header=None, names=['Class'])
valid_data = pd.read_csv(url_valid_data, sep='\s+', header=None)
valid_labels = pd.read_csv(url_valid_labels, sep='\s+', header=None, names=['Class'])

# Combine training and validation data
data = pd.concat([train_data, valid_data], axis=0).reset_index(drop=True)
labels = pd.concat([train_labels, valid_labels], axis=0).reset_index(drop=True)
...
# Transfer to numpy
y_test_kstartree = y_test['Class'].values
...
# Evaluate the model
accuracy_kstartree = model.evaluate(X_test, y_test_kstartree)
...
y_train_sknn = y_train.values.ravel()
y_test_sknn = y_test.values.ravel()
...
model.fit(X_train, y_train_sknn)
...
accuracy_sknn = accuracy_score(y_test_sknn, y_pred_sknn)
...
# Training Phase
model.fit(X_train, y_train_tensor)
...
# Evaluation
accuracy_fasbir = model.evaluate(X_test, y_test_tensor)
...
# Run LC-kNN
y_pred, _ = LCkNNModel(X_train_tensor, y_train_tensor, X_test_tensor, k=5, n_epochs=100, device=device)
...
# Evaluation
accuracy_gsknn = accuracy_score(y_test, y_pred)
...
output_dir = "outputs/feature_number/madelon"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "madelon.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "madelon.png")
