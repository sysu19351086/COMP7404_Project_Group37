...
url = "datasets/sample_size/blood/transfusion.data"
columns = ['Recency', 'Frequency', 'Monetary', 'Time', 'Donated']
data = pd.read_csv(url, names=columns)

# Ensure all data columns are of numeric type
for col in columns[:-1]:  # Excluding the target column 'Donated'
    data[col] = pd.to_numeric(data[col], errors='coerce')

# The 'Donated' column is the target variable
label_encoder = LabelEncoder()
data['Donated'] = label_encoder.fit_transform(data['Donated'])

X = data.drop('Donated', axis=1).values
y = data['Donated'].values

# Check for any NaNs or non-numeric values and handle them
...
output_dir = "outputs/sample_size/blood"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "blood.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "blood.png")
