...
# --------------------------------------------------------------------------------
# Data loading
path = "datasets/feature_number/libras/movement_libras.data"
data = pd.read_csv(path, header=None)

# Split features and target labels
X = data.iloc[:, :-1]  # Features (all columns but the last)
...
X = X.values
y = y.astype(np.int64)  # Ensure type is integer
...
output_dir = "outputs/feature_number/libras"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "libras.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "libras.png")
