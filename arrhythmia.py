...
# 加载数据并处理缺失值
path = "datasets/feature_number/arrhythmia/arrhythmia.data"
df = pd.read_csv(path, header=None, sep=',', na_values='?')

# 提取特征和标签
X = df.iloc[:, :-1]  # 前279列为特征
y = df.iloc[:, -1]    # 最后一列为标签

# 将标签转换为二分类问题：
# 1 = 正常心率，其他值（2-16）= 非正常心率
y = np.where(y == 1, 0, 1)  # 0=正常, 1=异常
...
adknn_model = ADkNNModel(base_k=5, alpha=0.1, device=device)
...
output_dir = "outputs/feature_number/arrhythmia"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "arrhythmia.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "arrhythmia.png")
