...
url = "datasets/sample_size/mice/Data_Cortex_Nuclear.csv"
...
excluded_cols = ['MouseID', 'Genotype', 'Treatment', 'Behavior', 'class']
numeric_cols = [col for col in data.columns if col not in excluded_cols]
...
X_train_tensor = torch.tensor(X_train.T, dtype=torch.float32).to(device)
X_test_tensor = torch.tensor(X_test.T, dtype=torch.float32).to(device)
...
output_dir = "outputs/sample_size/mice"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "mice.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "mice.png")
