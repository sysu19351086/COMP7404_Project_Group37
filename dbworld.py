...
# 加载数据集
with open('datasets/feature_number/dbworld/WEKA/dbworld_subjects.arff', 'r', encoding='latin-1') as f:
    data, meta = arff.loadarff(f)
df = pd.DataFrame(data)

# 提取特征和标签（假设最后一列为标签）
X = df.iloc[:, :-1].values.astype(np.float32)  # 确保特征为数值类型
y = df.iloc[:, -1].values
...
output_dir = "outputs/feature_number/dbworld"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "dbworld.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "dbworld.png")
