...
path = "datasets/sample_size/breast/breast-cancer-wisconsin.data"
columns = [
...
# Data cleaning: Replace question marks in the 'bare_nuclei' column with NaN and impute
data['bare_nuclei'] = pd.to_numeric(data['bare_nuclei'], errors='coerce')
data = data.fillna(data.median())  # Impute missing values with the median

# Convert class labels to numerical values, 1: benign, 2: malignant
data['class'] = data['class'].map({2: 0, 4: 1})

# Set features and target variable
X = data.drop(['id', 'class'], axis=1).values  # Features (excluding 'id' and 'class')
y = data['class'].values  # Target variable (0: benign, 1: malignant)
...
output_dir = "outputs/sample_size/breast"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "breast.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "breast.png")
