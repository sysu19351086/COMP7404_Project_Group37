...
path = "datasets/feature_number/lsvt/LSVT_voice_rehabilitation.csv"
data = pd.read_csv(path, header=None, skiprows=1)
data.dropna(axis=0, inplace=True)  # 删除缺失值
X = data.iloc[:, :-1].values  # The first column is the label, the rest are features
y = data.iloc[:, -1].values

# 将标签从1和2改为0和1
y = np.where(y == 1, 0, 1).astype(np.int32)
...
X_train_lcknn = X_train.astype(np.float32)   # 强制转换为 float32
X_test_lcknn = X_test.astype(np.float32)     # 强制转换为 float32
y_train_lcknn = y_train.astype(np.int64)     # 强制转换为 int64（对应 torch.long）
...
# Run LC-kNN
y_pred_lcknn, _ = LCkNNModel(X_train_lcknn, y_train_lcknn, X_test_lcknn, k=5, n_epochs=100, device=device)
...
output_dir = "outputs/feature_number/lsvt"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "lsvt.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "lsvt.png")
