...
url = "datasets/sample_size/australian/australian.dat"  # Ensure the file path is correct
columns = ['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'a9', 'a10', 'a11', 'a12', 'a13', 'class']
data = pd.read_csv(url, names=columns, sep='\s+')

# Define an improved preprocessing pipeline
preprocessor = ColumnTransformer(
    transformers=[
        ('cat', OneHotEncoder(sparse_output=False), [3, 4, 11]),  # Index starts from 0
        ('num', StandardScaler(), [1, 2, 6, 9, 12])               # Confirm feature indices are correct
    ],
    remainder='passthrough'  # Process other features (confirm these features are numerical)
)

X = data.drop('class', axis=1)
y = data['class'].values

# Execute preprocessing and convert data type
X_processed = preprocessor.fit_transform(X).astype(np.float32)  # Force conversion to float32
...
output_dir = "outputs/sample_size/australian"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Save Data to CSV File
csv_filename = os.path.join(output_dir, "australian.csv")
...
# Save Chart to File
image_filename = os.path.join(output_dir, "australian.png")
